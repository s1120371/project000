...
# --- GGUF AI 模型載入 (已移除，改用 Gemini API) ---
# 已瘦身：移除 Llama 模型載入程式碼
...
def generate_gemini_evaluation(user_profile, diet_data):
    ...
    # 1. 處理使用者資料
    profile_text = build_profile_text(user_profile, include_daily_goals=True)
    # 2. 處理飲食數據
    diet_text = f"""
...
# --- 核心AI功能函式 ---
def normalize_profile_value(value, mapping, fallback='未設定'):
    if value is None:
        return fallback
...
def format_allergens(allergens):
    ...
    if isinstance(allergens, list):
        return ', '.join(str(item).strip() for item in allergens if str(item).strip()) or '無'
    return str(allergens).strip() or '無'

def build_profile_text(user_profile, include_daily_goals=False):
    goal_map = {
        'weight-loss': '減重',
        'muscle-gain': '增肌',
        'control-sugar': '控制血糖',
        'general-health': '維持一般健康',
        '一般健康': '維持一般健康',
        '維持一般健康': '維持一般健康',
        '減重': '減重',
        '增肌': '增肌',
    ...
    lines = [
        f"- 健康目標: {normalize_profile_value(user_profile.get('goal'), goal_map)}",
        f"- 飲食習慣: {normalize_profile_value(user_profile.get('diet'), diet_map)}",
        f"- 已知過敏原: {format_allergens(user_profile.get('allergens'))}",
    ]

    if include_daily_goals:
        goal_water = user_profile.get('goalWater')
        goal_exercise = user_profile.get('goalExercise')
        if goal_water:
            lines.append(f"- 每日飲水目標: {goal_water} ml")
        if goal_exercise:
            lines.append(f"- 每日運動目標: {goal_exercise} 分鐘")
...
2-1. **資料判讀規則：只有使用者本人在對話中明確提供的新資訊可以覆蓋基本資料。過去 AI 助手曾經說過的「未設定」或推測，不可以覆蓋目前提供給你的使用者基本資料。**
...
6.  保持回覆簡潔、溫暖、易於理解。
7.  **格式限制：請輸出純文字。不要使用 Markdown 語法，不要用星號 (*) 做粗體或項目符號；若需要條列，請用「1. 2. 3.」數字編號。**
"""
    profile_text = build_profile_text(user_profile, include_daily_goals=True)
    ...
    try:
        ...
        print("正在呼叫 Google Gemini API (對話)...")
        result = llm_chat.invoke(langchain_messages)
        ...
        print(f"AI 回覆生成失敗 (Gemini): {e}")
        if original_creds:
            os.environ['GOOGLE_APPLICATION_CREDENTIALS'] = original_creds
        return "抱歉，AI 服務目前暫時無法連線。請稍後再試。"
